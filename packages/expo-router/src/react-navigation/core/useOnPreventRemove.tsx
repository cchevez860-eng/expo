...
type RouteOptions = { preventRemove?: boolean };

type Options = {
  getState: () => NavigationState;
  getRouteOptions: (routeKey: string) => RouteOptions | undefined;
...
export const shouldPreventRemove = (
  ...
  for (const route of getRemovedRoutes(currentRoutes, nextRoutes)) {
    ...
    if (getRouteOptions(route.key)?.preventRemove) {
      emitter.emit({
...
export function useOnPreventRemove({
  ...
  }, [addKeyedListener, emitter, getRouteOptions, getState, preventRemoveListeners, routeKey]);
